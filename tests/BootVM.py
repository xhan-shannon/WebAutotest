...
class BootVM(Basejob):
    ...
    def test_boot_vm_normally(self):
        u'''启动指定的已经存在虚拟机'''
        #
        ...
        vm_summary_page.execute_boot_system()
        
        # Step 4: Check the vm current status, it would turn into "in shutdown progress"
        check_return = utils_misc.wait_for(vm_summary_page.is_vm_starting, 10, 0.2, 0.1, "Check if vm goes into shutdown status")
        self.assert_(check_return, "After execution of boot up action, vm must go into 'in starting'")
