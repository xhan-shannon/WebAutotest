...
class ShutdownVM(Basejob):
    ...
    def test_shutdown_vm_normally(self):
        u'''删除指定的已经存在虚拟机'''
        #
        ...
        vm_summary_page.execute_shutdown_system(True)
        
        # Step 4: Check the vm current status, it would turn into "in shutdown progress"
        check_return = utils_misc.wait_for(vm_summary_page.is_vm_in_shutdown_progress, 10, 0.2, 0.1, "Check if vm goes into shutdown status")
