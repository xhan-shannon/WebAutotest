...
class RegisterISO(Basejob):
    ...
    def test_register_iso(self):
        u'''
                         ...
                         注册平台 后,注册ISO镜像用于安装虚拟机
         '''
        register_iso_config = utils_misc.readconfig(self.__class__.__name__)
        ...
        #         directory.
        iso_image_file_name = register_iso_config.get("iso", "redhat_iso")
        
        if not iso_image_file_name:
            iso_image_file_name = "RedHat_6.4_iso"
        
        iso_image_file_name = "RedHat_6.4_iso_fake"
        ...
        file_type_iso_radiobtn = pd_image_register_uimap.get_file_type_iso_radiobtn()
        
        if not file_type_iso_radiobtn:
            raise ElemNotExisted
        
        file_type_iso_radiobtn.click()
        time.sleep(TestSpeedControl.TEST_STEP_INTERVAL)
            
        # Step 6: select the proper OS type
        # Step 6: 选择合适的操作系统类型    
        os_type = register_iso_config.get("iso", "os_type")
        pd_image_register_uimap.select_ostype_by_value(os_type)
        
        time.sleep(TestSpeedControl.TEST_STEP_INTERVAL)
        os_name = register_iso_config.get("iso", "os_name")
        try:
        ...
        while curtime < time_out:
            ...
            try:
                ...
                if orig_window_handles != cur_window_handles:
                ...
                if pd_image_register_uimap.is_loading_in_registering():
                    continue
            finally:
                time.sleep(0.2)
                curtime = curtime + 1
